...
<div class="navbar-container" style="background: #1e1e24; padding: 16px 40px; display: flex; justify-content: space-between; align-items: center; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif; box-shadow: 0 2px 8px rgba(0,0,0,0.15); margin: -40px -40px 35px -40px; border-bottom: 1px solid #2d2d35;">
    <div style="display: flex; gap: 28px; align-items: center;">
        <?php if ($current_page === 'index.php' || $current_page === 'report.php' || $current_page === 'edit.php' || $current_page === 'add.php'): ?>
    ...
    <div style="color: #e2e2e8; font-size: 14px; display: flex; align-items: center; gap: 20px;">
        <?php if (isLoggedIn()): ?>
            <span style="letter-spacing: 0.2px;">Welcome, <strong style="color: #fff; font-weight: 600;"><?= htmlspecialchars(getUsername()) ?></strong> <span style="background: #3a3a44; color: #cbd5e1; font-size: 11px; padding: 3px 8px; border-radius: 12px; margin-left: 6px; font-weight: 500; text-transform: uppercase; letter-spacing: 0.5px;"><?= htmlspecialchars($_SESSION['role'] ?? 'staff') ?></span></span>
