...
<body>
    ...
    <div class="stats-container">
        ...
        <div class="stat-card low-stock-alert">
            ...
            <p><?= $stats_result['low_stock_count'] ?? 0 ?></p>
        </div>
    </div>
        ...
        <tbody>
            <?php if ($result && $result->num_rows > 0): ?>
                <?php while ($row = $result->fetch_assoc()): ?>
                    <?php $lowStockClass = ($row['stock'] < 20) ? 'class="row-low-stock"' : ''; ?>
                    <tr <?= $lowStockClass ?>>
                        ...
                        <td>
                            <?= htmlspecialchars($row['stock']) ?> units
                            <?= ($row['stock'] < 20) ? ' <small style="color:#d32f2f;">(ALERT)</small>' : '' ?>
                        </td>
                        <td><?= date('Y-m-d', strtotime($row['created_at'])) ?></td>
                        <td>
                            <?php if (isAdmin()): ?>
                                <a href="edit.php?id=<?= $row['id'] ?>" style="color: #2196F3; text-decoration: none; margin-right: 10px;">Edit</a>
                                <a href="delete.php?id=<?= $row['id'] ?>" style="color: #f44336; text-decoration: none;" onclick="return confirm('Delete this product?')">Delete</a>
                            <?php else: ?>
                                <span style="color: #f44336; font-size: 13px; font-weight: 500;">Access denied</span>
                            <?php endif; ?>
                        </td>
                    </tr>
                <?php endwhile; ?>
            <?php else: ?>
                <tr>
                    <td colspan="8" style="text-align: center; color: #999; padding: 30px;">No matching products found.</td>
                </tr>
            <?php endif; ?>
        </tbody>
    </table>
